...
xmlhttp.onreadystatechange = function () {
    if (xmlhttp.readyState == 4) {
        ...
        if (xmlhttp.status === 200) {
            ...
            window.socket = io("http:" + "//" + document.location.hostname + ":" + 3000);
            ready();
...
window.setTimeout(function () {
    ...
    if (!window.socket) {
        ...
        console.log("Using fallback port 80");
        window.socket = io();
        ready();
...
xmlhttp.open("GET", document.location.protocol + "//" + document.location.hostname + ":" + 3000, true);
xmlhttp.send();
